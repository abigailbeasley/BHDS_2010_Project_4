...
# adding death count
health_data$death_count <- health_data$val * 100000
...
county_health_data <- health_data %>%
  group_by(fips) %>%
  summarize(average_death_count = mean(death_count, na.rm=TRUE))
...
```{r}
filter_factors_county_level <- county_level %>%
  filter(sex=='Male' & year_id==1990 & age_group_name=='2 to 4')
```
...
plot_usmap(
  regions = "counties",
  data = county_health_data,
  values = "average_death_rate",
  color = "white",
  linewidth = 0.1
)
