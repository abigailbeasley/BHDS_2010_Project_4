...
# Ensuring that RStudio recognizes the correct file before beginning app
...
ui <- navbarPage("Overweight Population Trends",
                 ...
                 # Combined Summary and Map Page
                 tabPanel(
                   "Overweight Summary & Map",
                   fluidPage(
                     # Title for this page
                     ...
                     sidebarLayout(
                       sidebarPanel(
                         ...
                         # Adding metric selection (mean prevalence or increase in obesity)
                         ...
                         hr(),
                         # Summary statistics half of the page
                       ...
                       mainPanel(
                         ...
                         fluidRow(
                           column(
                             12,
                             # Text for summary statistics, including operation instructions
                 ...
                 tabPanel(
                   ...
                   fluidPage(
                     ...
                     p("The histogram displays the distirbution of overweight prevalence among different 
                       younger demographic groups in the United States. Users can select age groups and 
                       gender to examine how frequently certain overweight rates occur. This visualization 
                       helps identify whether overweight prevalence tends to cluster within certain ranges 
                       and reveals differences between population subgroups. Using the sliding year bar on 
                       the sidebar tab reveals that as the year changes, the distribution of the data changes
                       significantly for the subgroup of interest. For example, the year 1995 for females ages 
                       2-4 may be normally distributed, but the year 2025 may have right-sided skewness."),
                     #Sidebar with inputs
                     sidebarLayout(
                       sidebarPanel(
                 ...
                 tabPanel(
                   ...
                   fluidPage(
                     ...
                     # Text for this page
                     p("The time series plot shows how mean overweight prevalence changes 
                       over time for select age groups and genders per state. Users are able to 
                       observe trends such as rising or falling rates within specific populations. 
                       This plot is useful for identifying long-term patterns, comparing increases 
                       and decreases amongst states, and the changes brought on by certain initiatives. 
                       Furthermore, this plot is used to give a more in-depth view on the 'timeline' 
                       established within the overweight map."),
                     #Sidebar with inputs
                     sidebarLayout(
                       sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$combo_summaryTable <- DT::renderDataTable({
    ...
    summary_data <- state_level %>%
      ...
      # Filter the data by regions, age group, and gender
      filter(
        region %in% input$combo_sum_regions,
    ...
    summary_stats <- summary_data %>%
      ...
      summarise(
        Count = n(),
        # Adding nsmall, digits, and trim to keep figures displaying at 0.000, removes just 0 from occurring in the table
    ...
    if (input$toggle_view %% 2 == 1) {
      ...
      summary_stats_summary <- summary_stats %>%
        select(Year, Gender, `Age Group`, Region, Mean, Median, SD)
  ...
  output$histPlot <- renderPlotly({
    plot_data <- state_level %>%
      # Filtering data by year, age group, and gender
      filter(
        sex == input$hist_gender,
        year_id == input$hist_year,
        age_group_name == input$hist_age
      )
    # Creating the histogram plot using ggplot and plotly
  ...
  output$timeSeriesPlot <- renderPlotly({
    ...
    plot_data <- state_level %>%
      # Filtering the data by gender, age group, and state
      filter(
        sex == input$ts_gender,
    ...
    ts_plot <- ggplot(plot_data, aes(x = year_id, y = mean_prev, color = state)) +
      ...
      theme(
        ...
        axis.title = element_text(color = "white"),
        axis.text = element_text(color = "white"),
        legend.title = element_text(color = "white"), # Creating legend
        ...
        legend.position = "right", # Putting legend at the right side of the page
